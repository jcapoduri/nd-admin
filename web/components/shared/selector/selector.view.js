...
import template from './selector.tpl.html'
import Binding from '../../helpers/binding';

var SelectorItemView = Marionette.View.extend({
  ui: {
    item: 'li'
  },
  events: {
...
var SelectorView = Marionette.View.extend({
  behaviors: [Binding],
  template: template,
  ...
  events: {
    'keydown   @ui.searchinput': 'updateSearch',
  ...
  initialize: function(options) {
    ...
    this.buffer = new Backbone.Collection();
  ...
  guessValue: function(evt) {
    var value = this.model.get('search'),
        searchQuery = {};
    searchQuery[this.options.searchKey] = value.trim();
